...
Take a look at the program [matching.go](src/matching.go). Explain what happens and why it happens if you make the following changes. Try first to reason about it, and then test your hypothesis by changing and running the program.
  ...
  * What happens if you remove the `go-command` from the `Seek` call in the `main` function?
  1.	Det som sker är att varje värde som vi skickar in i for-loopen skickas vidare direkt. Därför kommer namnen alltid printas i samma ordning, i listans ordning. Med en Go routine så väntar inte for-loopen på att skicka utan skickar allt på direkten. Detta gör att namnen som dyker upp i channeln kommer i olika ordning varje gång.
  * What happens if you switch the declaration `wg := new(sync.WaitGroup`) to `var wg sync.WaitGroup` and the parameter `wg *sync.WaitGroup` to `wg sync.WaitGroup`?
  2.	När vi ändrar så skapas ett deadlock/ negativa värden på waitgroupen vilket skapar error. Detta på grund av att funktionen Seek inte får en pointer av waitgroup utan en faktsik variabel. Vi skulle kunna använda denna deklaration om vi satte i Seek wg *Sync.Waitgroup och att kalla på funktionen seek genomseek(&wg) istället för endast wg. Då vet waitgrupen att det är en pointer och den kör klart funktionen med värdena och avslutar efter.
  * What happens if you remove the buffer on the channel match?
  3.	Om man tar bort buffern så kommer en deadlock ske. Detta eftersom match endast ska ta emot ett värde (namn) om den skulle acceptera obegränsat som den gör om vi tar bort det vet inte programmet vilken som ska köras och dem olika värdena väntar på varandra.
  * What happens if you remove the default-case from the case-statement in the `main` function?
  4.	Ingenting. Default actions behövs inte alltid men oftast används dem genom att ha error meddelande eller likandande. Men i vårt fall tas ingen indata så det behövs inte.
